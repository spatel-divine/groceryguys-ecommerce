    ...
    public function updateappdetails(Request $request)
    {
      ...
      if($check){
        ...
        $update = DB::table('tbl_web_setting')
                ->update(['name'=> $app_name,'icon'=> $filePath, 'favicon'=>$filePath1, 'number_limit'=>$number_length,'last_loc'=>$last_loc]);
    
      }
      else{
          $update = DB::table('tbl_web_setting')
                ->insert(['name'=> $app_name,'icon'=> $filePath, 'favicon'=>$filePath1, 'number_limit'=>$number_length,'last_loc'=>$last_loc ]);
      }
     if($update){
        return redirect()->back()->withSuccess(trans('keywords.Updated Successfully'));
     }
