...
class StoreloginController extends Controller
{
 public function store_login(Request $request)
    ...
    {   
        ...
        $store_password = $request->password;
        	 if($request->store_image){
            $user_image = $request->store_image;
            $fileName = $user_image->getClientOriginalName();
            $fileName = str_replace(" ", "-", $fileName);
            $user_image->move('images/store/'.$date.'/', $fileName);
    ...
    if($update){
         $store=  DB::table('store')
                ->where('id', $store_id )
                ->first();
        	$message = array('status'=>'1', 'message'=>'Store Profile Updated', 'data'=>$store);
	        return $message;
              }
    	else{
